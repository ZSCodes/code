...
//敌方投掷过程
export class EnemyThrow extends Sprite {
    constructor() {
        ...
        this.throw_x = DataStore.getInstance().width * 0.72;   //投掷过程的X轴坐标（起始位置）
        this.throw_y = DataStore.getInstance().height * 0.07;   //投掷过程的Y轴坐标（起始位置）
    ...
    draw() {
        //投掷过程
        if (DataStore.getInstance().throw_state == 'enemy') {
            let t;  //投掷的移动速度
            ...
            if (DataStore.getInstance().throw_x >= DataStore.getInstance().throw_y) {
                t = (this.throw_x - DataStore.getInstance().throw_x) / 4;
            } else {
                t = (DataStore.getInstance().throw_y - this.throw_y) / 4;
            }
            ...
            if (this.throw_x > DataStore.getInstance().throw_x) {
                this.throw_x = this.throw_x - ((this.throw_x - DataStore.getInstance().throw_x) / t);
            }
            if (this.throw_y < DataStore.getInstance().throw_y) {
                this.throw_y = this.throw_y + ((DataStore.getInstance().throw_y - this.throw_y) / t);
            }
            if (this.throw_x <= DataStore.getInstance().throw_x && this.throw_y >= DataStore.getInstance().throw_y) {
                ...
                this.throw_x = DataStore.getInstance().width * 0.72;
                this.throw_y = DataStore.getInstance().height * 0.07;
                this.throw_width = DataStore.getInstance().width * 0.13;
                this.throw_height = DataStore.getInstance().width * 0.13;
                Director.getInstance().createEnemyAim(DataStore.getInstance().hit_x, DataStore.getInstance().hit_y);
